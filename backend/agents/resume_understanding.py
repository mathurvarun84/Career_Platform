...
class ResumeUnderstandingAgent(BaseAgent):
    ...
    def run(self, input_dict: dict) -> dict:
        ...
        system_prompt = (
            ...
            "  sub_entries: experience=one per company/role block (EVERY employer — do not "
            "  merge or skip roles; a 15-year career typically has 5-8+ entries), "
            "  education=one per degree, certifications=one per cert. "
            "  summary/skills/awards: sub_entries=[]\n"
